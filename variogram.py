...
def compare_output_helper(data_type, component, timestep, i, plot=False):
    ...
    cub = "PhIREGAN/{data_type} test/bicubic5/bicubic_{component}_{timestep}_{i}.png".format(data_type=data_type, component=component, timestep=timestep, i=i)
    edsr = "PhIREGAN/{data_type} test/sr_output_solar/{component}_{timestep}_{i}.png".format(data_type=data_type, component=component, timestep=timestep, i=i)
    ...
    if os.path.isfile(cub) and os.path.isfile(edsr) and os.path.isfile(gans):
...
def plot_variogram():
    ...
    plt.title("DNI Semivariogram")
    plt.legend()
    plt.savefig("dni_semivariogram.png", dpi=1000, transparent=True, bbox_inches='tight')
    plt.show()
