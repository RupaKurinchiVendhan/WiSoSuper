...
from ESRGAN.lib.train_module import Network
from ESRGAN.lib.utils import create_dirs, de_normalize_image, load_inference_data
...
def set_flags():
    ...
    Flags.DEFINE_string('inference_result_dir', './output', 'output directory during inference')
...
def test(data_dir, save_dir):
    FLAGS = set_flags()

    # make dirs
    target_dirs = [save_dir]
    create_dirs(target_dirs)

    # load test data
    LR_inference, LR_filenames = load_inference_data(data_dir, FLAGS)
